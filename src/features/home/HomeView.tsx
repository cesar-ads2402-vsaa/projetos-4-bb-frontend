...
import { TutorialCard } from "../media/components/TutorialCard";
import { useMediaView } from "../media/hooks/useMediaView";
                ...
                <Box bg="white" p={4} borderRadius="xl" shadow="md" borderWidth="1px" w="full">
                    <VStack align="start" gap={2} w="full">
                        ...
                        <HStack w="full" gap={2} overflowX="auto" pb={1} css={{ scrollbarWidth: "none", "&::-webkit-scrollbar": { display: "none" } }}>
                            ...
                            <Button size="sm" borderRadius="full" flexShrink={0} variant={!estados.selectedLanguage ? "solid" : "subtle"} colorPalette={!estados.selectedLanguage ? "blue" : "gray"} onClick={() => acoes.setLanguage("")} px={4} fontWeight="medium">
                                Explorar Todos
                            </Button>
                ...
                <Box w="full">
                    <SimpleGrid columns={5} gap={2} maxW="full">
                        {categoriasComOutros.map((cat) => {
                            ...
                            return (
                                <VStack key={cat.nome}>
                                    <Circle size={{ base: "60px", md: "80px" }} bg={isAtivo ? cat.color : "white"} color={isAtivo ? "white" : cat.color} shadow="lg" cursor="pointer" transition="all 0.3s" onClick={() => setFiltroAtivo(isAtivo ? null : cat.nome)} _hover={{ transform: "scale(1.1)", bg: isAtivo ? cat.color : cat.bg }}>
                                        <Icon as={cat.icon} fontSize={{ base: "24px", md: "30px" }} />
                                    </Circle>
                                    <Text fontSize="xs" fontWeight="bold" color="blue.900" mt={1}>{cat.nome}</Text>
                                </VStack>
                            );
                        })}
                    </SimpleGrid>
            ...
            <Box w="full" py={12} px={{ base: 4, md: 8, "2xl": 12 }}>
                <Flex justify="space-between" align="end" mb={10}>
                    <VStack align="start" gap={1}>
                        ...
                        <Heading size="lg" color="blue.900" fontWeight="800">Tutoriais Disponíveis</Heading>
                    </VStack>
                    ...
                    <SimpleGrid columns={{ base: 1, sm: 2, md: 3, lg: 4, "2xl": 5 }} gap={10} w="full">
                        {tutoriaisExibidos.map((tut) => (
                            <Box key={tut.id} transition="0.3s" _hover={{ opacity: 0.9 }}>
                                <TutorialCard tutorial={tut} onClick={() => handleVideoClick(tut.pergunta)} />
                            </Box>
                        ))}
                    </SimpleGrid>
                ) : (
